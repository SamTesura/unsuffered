...
void main() {
    ...
    // Edge detection
    // TRELLIS models have noisy normals — thresholds are tuned to only fire
    // at real silhouette boundaries and limb creases, not per-vertex noise.
    // Improved model has 50.6% outward normals, so we can be slightly more
    // responsive for better definition without flooding the mesh with outlines.
    float normal_edge = sobel_normal(v_texcoord, texel) * u_thickness;
    float depth_edge = sobel_depth(v_texcoord, texel) * u_thickness * 0.5;

    // Outer edge: silhouette boundaries + depth discontinuities
    float outer_edge = max(normal_edge * 0.45, depth_edge);
    outer_edge = smoothstep(0.10, 0.28, outer_edge);

    // Inner edge: feature boundaries (creases between limbs, folds)
