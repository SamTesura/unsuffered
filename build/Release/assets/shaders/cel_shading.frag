...
// NOTE: TRELLIS AI-generated models have noisy normals. Naive SSAO treats
// this noise as geometry crevices, darkening the ENTIRE model. We use a wider
// sampling radius and threshold to only detect real concavities.
// TUNED for new model with improved normals (50.6% outward):
//   - Tighter sampling radius (3.5 from 4.0) since normals are cleaner
//   - More sensitive threshold to catch real folds
//   - Stronger darkening now that false positives are reduced
float compute_ssao_hint(vec2 uv) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        // Threshold: ignore small angular differences (noise) — only count real crevices
        diff = smoothstep(0.25, 0.70, diff);
        occlusion += diff;
    }

    occlusion /= 4.0;
    return 1.0 - occlusion * 0.40;  // Stronger darkening for improved normals
}
...
void main() {
    ...
    // 0.25 ensures clear visibility even with backward-facing normals.
    cel = max(cel, 0.25);
    ...
    // Minimum brightness floor: lowered since textures now provide visual
    // interest — we don't need as high a floor to prevent silhouettes.
